...
class WidgetInfo(BaseModel):
    """Lean widget instance metadata returned from the LSP widget RPC."""

    id: str = Field(description="Widget instance ID")
    name: Optional[str] = Field(None, description="Widget type name")
    javascript_hash: Optional[str] = Field(
        None, description="Hash for the widget JavaScript module"
    )
    props: Optional[Dict[str, Any]] = Field(
        None, description="Widget props payload from Lean"
    )
...
class HoverInfo(BaseModel):
    ...
    widgets: List[WidgetInfo] = Field(
        ...
        description="Widget instances at this position (if requested)",
    )
