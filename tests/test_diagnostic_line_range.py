...
@pytest.fixture()
def namespaced_diagnostic_file(test_project_path: Path) -> Path:
    """Create a test file with namespaced declarations to test nested symbol search."""
    path = test_project_path / "NamespacedDiagnosticTest.lean"
    content = textwrap.dedent(
        """
        import Mathlib
...
@pytest.mark.asyncio
async def test_diagnostic_messages_with_namespaced_declaration(
    mcp_client_factory: Callable[[], AsyncContextManager[MCPClient]],
    namespaced_diagnostic_file: Path,
) -> None:
    """Test filtering diagnostics by a declaration inside a namespace."""
    async with mcp_client_factory() as client:
        # Get all diagnostics first
        all_diagnostics = await client.call_tool(
            "lean_diagnostic_messages",
            {"file_path": str(namespaced_diagnostic_file)},
        )
        all_diag_text = result_text(all_diagnostics)
        ...
        # Get diagnostics for namespaced theorem using unqualified name
        diagnostics = await client.call_tool(
            "lean_diagnostic_messages",
            {
                "file_path": str(namespaced_diagnostic_file),
                "declaration_name": "namespacedTheorem",
            },
        )
        diag_text = result_text(diagnostics)

        # Should find the namespaced declaration and return its diagnostics
        assert len(diag_text) > 0


@pytest.mark.asyncio
async def test_diagnostic_messages_namespaced_declaration_no_errors(
    mcp_client_factory: Callable[[], AsyncContextManager[MCPClient]],
    namespaced_diagnostic_file: Path,
) -> None:
    """Test filtering by a valid namespaced declaration with no errors."""
    async with mcp_client_factory() as client:
        # Get diagnostics for valid namespaced definition
        diagnostics = await client.call_tool(
            "lean_diagnostic_messages",
            {
                "file_path": str(namespaced_diagnostic_file),
                "declaration_name": "namespacedDef",
            },
        )
        diag_text = result_text(diagnostics)

        # Should find the declaration but have no errors
        assert (
            "no" in diag_text.lower()
            or len(diag_text.strip()) == 0
            or diag_text == "[]"
