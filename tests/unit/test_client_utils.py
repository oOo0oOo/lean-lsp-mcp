...
def test_serial_pool_is_single_slot_and_separate(tmp_path: Path) -> None:
    project = _make_project(tmp_path / "proj")
    client = _MockAioClient(project)
    attach_shared_client(project, client)
    ctx = _Context(_LifespanContext(project, client))

    serial_pool = get_serial_scratch_pool(ctx)
    scratch_pool = get_scratch_pool(ctx)

    assert serial_pool is get_serial_scratch_pool(ctx)
    assert serial_pool is get_run_code_pool(ctx)
    assert serial_pool is not scratch_pool
    assert serial_pool._size == 1
    assert serial_pool._paths == ["_mcp_serial_0.lean"]
